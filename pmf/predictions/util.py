...
def s3_move(bucket_name, source_path, target_path, aws_access_key_id, aws_secret_access_key, endpoint_url, add_date_to_filename=False):
...
def local_to_s3(host, ACCESS_KEY,SECRET_KEY,bucket_name,root_path,s3_destination_path):
    ...
    for path, subdirs, files in os.walk(root_path):
        ...
        s3_destination_path = s3_destination_path.replace(bucket_name+'/',"")
        for file in files:
            my_bucket.upload_file(os.path.join(path, file), os.path.join(s3_destination_path,directory_name)+'/'+file)
    try:
...
def execute_hive_queries(queries, pmf_config):
    ...
    conn = jaydebeapi.connect(driver, url, driver_args={"user": pmf_config["thrift_user"], "password": pmf_config["thrift_password"]})
    cursor = conn.cursor()
    for q in queries:
        cursor.execute(q)
...
def get_cfg_data(s3_endpoint, s3_access_key, s3_secret_key, s3_bucket, s3_config_file_path):
    import traceback
    s3 = boto3.resource('s3', endpoint_url='http://' + s3_endpoint, aws_access_key_id=s3_access_key,
...
def create_s3_connection(endpoint_url, aws_access_key_id, aws_secret_access_key, s3_bucket_name):
...
def write_excel_in_s3(df,endpoint,access_key,secret_key,s3_bucket_name,config):
    now = dt.datetime.now()
    output = io.BytesIO()
    writer = pd.ExcelWriter(output, engine='openpyxl')
    df.to_excel(writer, index=False,sheet_name="Overlay")
    writer.save()
    path = str(config['s3_output_folder_path']) +"/year="+str(now.year)+"/month="+str(now.month)+"/gc_allocation_tmo_"+ str(dt.datetime.now().strftime('%d%m%Y'))+".xlsx"
    data = output.getvalue()
    s3 = boto3.resource('s3',endpoint_url='http://'+endpoint, aws_access_key_id=access_key,
                        aws_secret_access_key=secret_key)
